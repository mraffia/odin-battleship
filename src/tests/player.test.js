...
test('getFleet() will return all ships the player has', () => {
  ...
  const fleet = playerOne.getFleet();
...
test("randomAttack(enemy) will attack a random coordinate on enemy's board", () => {
  ...
  const attackResult = playerOne.randomAttack(enemy);
  const enemyPlayerBoard = enemy.getPlayerBoard().getBoard();

  expect(enemyPlayerBoard[attackResult[0]]).toBe('x');
});
